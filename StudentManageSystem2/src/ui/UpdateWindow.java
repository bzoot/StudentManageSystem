...
public class UpdateWindow extends JFrame {
	...
	public UpdateWindow(final ManageSystemImp manageSystem, final QueryPage qp,
			final Student st) {
		setLayout(null);
		setVisible(true);
		setAlwaysOnTop(true);
		...
		lb.add(idSchool);
		final JTextField idSchoolF = new JTextField(String.valueOf(st
				.getIdSchool()));
		...
		lb.add(name);
		final JTextField nameF = new JTextField(st.getName());
		...
		lb.add(age);
		final JTextField ageF = new JTextField(String.valueOf(st.getAge()));
		...
		lb.add(gender);
		final JTextField genderF = new JTextField(st.getGender());
		...
		lb.add(birthday);
		final JTextField birthdayF = new JTextField(
				sdf.format(st.getBirthday()));
		...
		lb.add(birthplace);
		final JTextField birthplaceF = new JTextField(st.getBirthplace());
		...
		lb.add(hometown);
		final JTextField hometownF = new JTextField(st.getHometown());
		...
		lb.add(nation);
		final JTextField nationF = new JTextField(st.getNation());
		...
		lb.add(id);
		final JTextField idF = new JTextField(String.valueOf(st.getId()));
		...
		lb.add(health);
		final JTextField healthF = new JTextField(st.getHealth());
		...
		lb.add(phoneNumber);
		final JTextField phoneNumberF = new JTextField(String.valueOf(st
				.getPhoneNumber()));
		...
		submit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				} else if (birthdayF.getText() == null
						|| birthdayF.getText().isEmpty()) {
					birthday = sdf.format("0001-01-01");
				} else {
					new ErrorPopFrame(thiz);
					return;
				}
				...
				try {
					if (manageSystem.updateStudent(st.getIdSchool(),
							Long.parseLong(idSchool), name, gender,
							Integer.parseInt(age), sdf.parse(birthday),
